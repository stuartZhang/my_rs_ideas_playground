...
/// 涉及到的功能点包括
/// （1）修改·被代理成员方法·的【方法名】
/// （2）修改·被代理成员方法·的【形参类型】— 类型转换·需要·`From / TryFrom trait`实现。
/// （3）修改·被代理成员方法·的【返回值类型】— 类型转换·需要·`From / TryFrom trait`实现。
/// （4）忽略·被代理成员方法·的【返回值】
/// （5）将【成员方法】委托于不同的【字段值】或【字段值·表达式·返回值】
mod data_structure {
    ...
    impl Wrapper {
        delegate! {
            to self.inner1 { // #5. 委托至【指定·字段】上的【成员方法】
                #[call(push)] // #1. 修改·被代理成员方法名`push -> add`
                pub fn add(&mut self, value: u32);
                #[call(len)]
                #[try_into(unwrap)] // #3. 修改·被代理成员方法·的返回值类型`usize -> u64`。
            ...
            to self.inner2.borrow_mut() { // #5. 委托至【指定·字段·表达式·返回值】上的【成员方法】
            ...
            to self.size {
                #[into] // #3. 修改·被代理成员方法·的返回值类型`i32 -> i64`。
