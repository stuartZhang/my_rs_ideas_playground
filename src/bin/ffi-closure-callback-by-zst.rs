 ...
 *      void add_two_numbers_by_val(int a, int b, Callback cb, void *closure) {...}
 *    Rust 端
 *      ffi_closure_shim_fn!(
 *          // （被导入）外部函数名
 *          add_two_numbers_by_val(
...
fn main() {
    use libc::c_int;
    ffi_closure_shim_fn!(
        add_two_numbers_by_val(a: c_int, b: c_int,, callback(result: c_int))
    );
    let mut got = 0;
    add_two_numbers_by_val(1, 2, |result: c_int| got = result);
    assert_eq!(got, 1 + 2);
}