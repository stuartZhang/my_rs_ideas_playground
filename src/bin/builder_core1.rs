...
mod factory {
    ...
    #[builder(no_std, derive(Serialize))]
...
fn main() {
    ...
    let x: Lorem = builder.my_name1(10001).my_name2(2002).build().unwrap();
    assert_eq!(x, Lorem::new(10001, 2002));
    // `TBuilder`实例自身就具有被序列化为`json`字符串的能力。
    assert_eq!(serde_json::to_string(&builder).unwrap(), r#"{"my_rename1":10001,"myName2":2002}"#);
}
